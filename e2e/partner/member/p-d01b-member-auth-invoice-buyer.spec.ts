// @partner-id P-D01B
// @features F-M-17, F-S-06
// @path Partner — buyer auth invoice financial rows
...
test.describe("P-D01B buyer auth invoice financial contract", () => {
  test("buyer invoice financial rows match DB snapshot", async ({
    page,
  }, testInfo) => {
    test.skip(testInfo.project.name !== "buyer", "Buyer-only invoice financial");
    test.skip(
    ...
    await assertMemberAuthInvoiceMatchesSnapshot(page, snapshot, "buyer");
  });
});
