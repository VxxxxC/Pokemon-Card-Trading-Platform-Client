...
import {
  PHASE4_EMAIL_CATALOG,
  PHASE4_EVENT_IDS,
  PHASE4_TEMPLATE_KEYS,
} from "@/lib/notifications/email-phase4-registry";
...
describe("Phase 4 email gate", () => {
  ...
  it("catalog has unique event ids and template keys", () => {
    expect(PHASE4_EMAIL_CATALOG.length).toBe(6);
    expect(new Set(PHASE4_EVENT_IDS).size).toBe(PHASE4_EVENT_IDS.length);
    expect(new Set(PHASE4_TEMPLATE_KEYS).size).toBe(PHASE4_TEMPLATE_KEYS.length);
  });

  it("renders every Phase 4 template", () => {
    for (const entry of PHASE4_EMAIL_CATALOG) {
  ...
  it("enqueues every Phase 4 catalog row", async () => {
    for (const entry of PHASE4_EMAIL_CATALOG) {
