...
import {
  PHASE3_EMAIL_CATALOG,
  PHASE3_EVENT_IDS,
  PHASE3_TEMPLATE_KEYS,
} from "@/lib/notifications/email-phase3-registry";
...
describe("Phase 3 email gate", () => {
  ...
  it("catalog has unique event ids and template keys", () => {
    expect(PHASE3_EMAIL_CATALOG.length).toBeGreaterThanOrEqual(25);
  ...
  it("renders every Phase 3 template", () => {
    for (const entry of PHASE3_EMAIL_CATALOG) {
  ...
  it("enqueues every Phase 3 catalog row", async () => {
    for (const entry of PHASE3_EMAIL_CATALOG) {
