...
| **`/admin/settings`** | `commissionRate`, `appraisalFee`, `fpsFee`, `maxWithdrawalLimit`, `termsText` | `platform_settings` <br>`key: platform_financial_config` | `getPlatformSettings()` <br>`updatePlatformSettings(settings)` | Strict Admin Fail-Closed (`is_admin()`) |
