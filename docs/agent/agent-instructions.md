...
1. [docs/implementation-guidelines.md](../implementation-guidelines.md) - 專案結構概述
2. [docs/dev/server.md](../dev/server.md) — 伺服器端 TODO 追蹤器
3. [docs/dev/api.md](../dev/api.md) — API 整合 TODO 追蹤器
4. [docs/dev/database.md](../dev/database.md) — 資料庫架構與查詢 TODO 追蹤器
5. [docs/dev/follow-up/](../dev/follow-up/) — 後期developement需要follow up的tasks
...
### 6. 測試資安防線與憑證保護鐵律 (Credential Safeguard Mandate)
...
> **報告根路徑定義**：本文件與 `opencode.json` 內的 `$Project_Root` 均指目前代碼庫（repository/workspace）的專案根目錄，與既有 `$Project_Root` 指向同一實體位置，禁止建立名為 `$Project_Root` 的字面資料夾。
...
- 各測試 agent 的報告一律寫入 `$Project_Root/test-results/[相關task名稱]/` 目錄下對應的 markdown 檔案，不得互相覆寫。
- Commander 在任務結束前，**必須強制讀取** `$Project_Root/test-results/[相關task名稱]/` 內今次 run 產生的所有報告，不可只信任 sub-agent 的口頭 summary。
