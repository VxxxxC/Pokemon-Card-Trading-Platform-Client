...
| `merchant_auth` / direct + auth toggle | Manual multicapture ⏸ hold | T+7 → **Stripe Connect** |
| `member_auth` | Manual multicapture ⏸ hold | T+3 → **FPS `payout_requests`** (not Connect) |

Auth multicapture E2E paused until Stripe approves account feature. Test **non-auth merchant** checkout + T+7 Connect path first.
