...
| Member auth escrow (C2C 鑑定託管) | ✅ Ready | 🟡 Stripe 已接駁，樣式待精修 | `app/actions/member-auth-checkout.ts`, `app/actions/orders.ts`, `rpc_prepare_member_auth_order_payment` / `rpc_mark_member_auth_order_paid`, migration **`20260729150000`**, `lib/payments/member-auth-payment.ts`, migrations **`20260708100000`**–**`20260708120000`** | `MemberOrderDetailView.tsx`, `MemberAuthStripePaymentPanel.tsx`, `MemberAuthMockPaymentPanel.tsx`（dev fallback）, `OfferCard.tsx`, `UserOrderRow.tsx` | [member-auth-escrow](./follow-up/member-auth-escrow/backend.md) · [member-auth-checkout](./follow-up/member-auth-checkout/frontend.md) — P2P 無鑑定不接 Stripe |
...
| Merchant checkout — 平台 Stripe 收款（B2C 託管，Milestone 1） | ✅ Ready | 🟡 已接駁，樣式待精修 | `app/actions/merchant-checkout.ts`, `app/actions/buy-now.ts`, `app/api/stripe/webhook/route.ts`（`payment_intent.*`）, `lib/stripe/env.ts`, `lib/merchant-checkout/pricing.ts`, `lib/stripe/payout-ready.ts`, RPC `rpc_buy_now_listing` / `rpc_prepare_merchant_order_payment` / `rpc_attach_merchant_order_payment_intent` / `rpc_mark_merchant_order_paid`, migrations **`20260729100000`**–**`20260729140000`** | `app/checkout/[id]/page.tsx`（Payment Element）, `app/checkout/[id]/success/page.tsx`, `BuyNowConfirmDialog.tsx`, `OfferCard.tsx`（前往付款）, `UserOrderRow.tsx` / `UserTradingClient.tsx`（待付款 CTA）, `MemberOrderDetailView.tsx`, `MerchantOrderDetailView.tsx`, `MerchantOrderRow.tsx` | [backend](./follow-up/merchant-checkout/backend.md) · [frontend](./follow-up/merchant-checkout/frontend.md) · [buy-now-chat](./follow-up/buy-now-chat/frontend.md) — ⚠️ 需加 `NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY`；⏳ Milestone 2 撥款 / 佣金 / 未付款過期 |
