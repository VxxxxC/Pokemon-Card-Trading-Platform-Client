...
| Merchant checkout — 平台 Stripe 收款 + Connect 撥款（B2C，Milestone 1–2） | ✅ Ready | 🟡 已接駁，樣式待精修 | `merchant-checkout.ts`, payout saga, `expire-merchant-pending-payment` cron, migrations **`20260729100000`**–**`20260803120600`**, `lib/marketplace/enrich-merchant-shipping.ts` | `checkout/[id]`, buyer B2C detail (`MemberMerchantB2cOrderInvoice`), merchant settings base fee, listing extra fee, **大盤運費展示**, **AddAssetModal 附加運費**, **交收資料持久化** | [backend](./follow-up/merchant-checkout/backend.md) · [frontend](./follow-up/merchant-checkout/frontend.md) — 商戶自定運費；固定 8%；⏳ 動態費率 / refund / 24h 付款提醒 |
