...
| Admin grading workbench（Member C2C + Merchant B2C 鑑定） | ✅ P1 Ready | ⏸ Paused | `app/actions/admin-grading.ts`, `lib/payments/*-capture-saga.ts`, `lib/payments/auth-grading-fail-void-saga.ts`, migrations **`20260731100000`**, **`20260730100000`**, **`20260729190000`**, `app/api/stripe/webhook/route.ts` | `/admin/grading`, `AdminGradingClient.tsx`（fault_party + void fail）, `MerchantOrderDetailView.tsx` | [backend](./follow-up/admin-grading/backend.md) · [escrow-payment-policy](./escrow-payment-policy.md) §18 P1 — ⏸ 待 Stripe 開通 online multicapture 後再續（見 admin-grading/backend.md） |
...
| Admin payouts（財務與結算） | ✅ Ready | 🟡 Partial | `app/actions/admin-payouts.ts`, `lib/admin-payouts/*`, migrations **`20260801120000`**, **`20260802120000`**, `app/api/cron/member-fps-payout-ready`, `scripts/dev/seed-fps-payout-requests.sql` | `/admin/payouts`, `FpsLedgerTab.tsx`, `MemberConnectLedgerTab.tsx` | [plan](./follow-up/admin-payouts/plan.md) · [backend](./follow-up/admin-payouts/backend.md) · [frontend](./follow-up/admin-payouts/frontend.md) — Merchant T+7 held ledger ✅; FPS ledger MVP ✅; auth multicapture E2E 仍待 Stripe 開通 |
...
- **Stripe multicapture:** Auth orders (C2C + Merchant B2C) require account-level **online multicapture** before admin grading E2E and member FPS payout full flow. PI must use `request_multicapture: if_available` (see `member-auth-checkout` / `merchant-checkout` follow-up).
