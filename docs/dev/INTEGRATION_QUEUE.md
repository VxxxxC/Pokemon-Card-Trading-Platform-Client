...
| Merchant trading orders (seller list + detail) | ✅ Ready | 🟡 Logistics UI aligned | `search_merchant_trading_orders`, `getMerchantOrderDetail`, `submitMerchantLogistics`, migration **`20260731120000`** | `merchant/trading/*`, `MerchantOrderDetailView.tsx` | [backend](./follow-up/merchant-trading/backend.md) · [frontend](./follow-up/merchant-trading/frontend.md) |
...
| Member auth escrow (C2C 鑑定託管) | ✅ P0 Ready | 🟡 Stripe manual capture 已接駁，樣式待精修 | `app/actions/member-auth-checkout.ts`, `lib/payments/auth-capture-saga.ts`, `rpc_mark_member_auth_order_authorized` / `rpc_finalize_auth_fee_capture`, migration **`20260730100000`**, migrations **`20260729150000`**, **`20260708100000`**–**`20260708120000`** | `MemberOrderDetailView.tsx`, `MemberAuthStripePaymentPanel.tsx`, `OfferCard.tsx`, `UserOrderRow.tsx` | [member-auth-checkout/backend](./follow-up/member-auth-checkout/backend.md) · [escrow-payment-policy](./escrow-payment-policy.md) §18 P0 |
...
| Merchant KYC flow (apply → admin review → Stripe Connect) | ✅ Ready | 🟡 Wizard + admin 表格 UI 已接駁；Stripe CTA 待精修 | `rpc_submit_merchant_kyc_application`（provisional merchant on submit）, `lib/kyc/merchant-gates.ts`, `lib/stripe/payout-ready.ts`, migrations **`20260728100000`**–**`20260728150000`** | `MerchantApplyClient.tsx`（submit → `/profile/merchant`）, `MerchantOverviewClient.tsx`（pending banner） | [backend](./follow-up/merchant-kyc/backend.md) · [frontend](./follow-up/merchant-kyc/frontend.md) |
| Merchant checkout — 平台 Stripe 收款 + Connect 撥款（B2C，Milestone 1–2） | ✅ Ready | 🟡 已接駁，樣式待精修 | `merchant-checkout.ts`, payout saga, `expire-merchant-pending-payment` cron, migrations **`20260729100000`**–**`20260729180000`**, **`20260731120000`** | `checkout/[id]`, buyer B2C detail (`MemberMerchantB2cOrderInvoice`) | [backend](./follow-up/merchant-checkout/backend.md) · [frontend](./follow-up/merchant-checkout/frontend.md) — 固定 8%；⏳ 動態費率 / refund / 24h 付款提醒 |
| Admin grading workbench（Member C2C + Merchant B2C 鑑定） | ✅ P1 Ready | 🟡 Baseline UI wired | `app/actions/admin-grading.ts`, `lib/payments/*-capture-saga.ts`, `lib/payments/auth-grading-fail-void-saga.ts`, migrations **`20260731100000`**, **`20260730100000`**, **`20260729190000`**, `app/api/stripe/webhook/route.ts` | `/admin/grading`, `AdminGradingClient.tsx`（fault_party + void fail）, `MerchantOrderDetailView.tsx` | [backend](./follow-up/admin-grading/backend.md) · [escrow-payment-policy](./escrow-payment-policy.md) §18 P1 |
| Admin dashboard data（數據總覽） | ✅ Ready | 🟡 Partial | `app/actions/admin-dashboard.ts`, `lib/admin-dashboard/*`, `lib/stripe/platform-balance.ts` | `/admin/dashboard`, `DashboardClient.tsx` | [plan](./follow-up/admin-dashboard/plan.md) · [backend](./follow-up/admin-dashboard/backend.md) · [frontend](./follow-up/admin-dashboard/frontend.md) — Phase 1–2: DB metrics + Stripe balance + reports alert; ⏳ health probes |
| Admin payouts（財務與結算） | ✅ Ready | 🟡 Partial | `app/actions/admin-payouts.ts`, `lib/admin-payouts/*`, migrations **`20260801120000`**, **`20260802120000`**, `app/api/cron/member-fps-payout-ready`, `scripts/dev/seed-fps-payout-requests.sql` | `/admin/payouts`, `FpsLedgerTab.tsx`, `MemberConnectLedgerTab.tsx` | [plan](./follow-up/admin-payouts/plan.md) · [backend](./follow-up/admin-payouts/backend.md) · [frontend](./follow-up/admin-payouts/frontend.md) — Merchant ledger ✅; FPS ledger MVP ✅; Member FPS pipeline ✅ |
| Member FPS payout (C2C auth seller) | ✅ Ready | ✅ Wired | `app/actions/profile.ts` (`updateUserFpsId`), `app/actions/orders.ts` (payout fields), migrations **`20260802120000`**, **`20260803120000`**, cron **`/api/cron/member-fps-payout-ready`** | `FpsIdCollectDialog.tsx`, `MemberOrderDetailView.tsx`, `UserSettingsClient.tsx` | [backend](./follow-up/member-fps-payout/backend.md) · [frontend](./follow-up/member-fps-payout/frontend.md) · **[Stripe E2E checklist](./follow-up/member-fps-payout/e2e-checklist.md)** |
| Buy now → confirm → chat（商戶 + 會員 P2P） | ✅ Ready | 🟡 已接駁，樣式待精修 | `app/actions/buy-now.ts`, `rpc_buy_now_listing`, migration **`20260729140000`**, `lib/chat/open-buy-now-session.ts`, `getOfferCardContext` order join | `BuyNowConfirmDialog.tsx`, `GlobalTxButtons.tsx`, `ExecutionSlideOver.tsx`, `OfferCard.tsx`, `NewArrivals.tsx`, `PremiumMarket.tsx` | [backend](./follow-up/buy-now-chat/backend.md) · [frontend](./follow-up/buy-now-chat/frontend.md) |
