...
| Auth Escrow v2 (鑑定兩段順豐 · single capture · fail 追償) | 🟢 Phase B + v2.1 + **Phase C** Ready | 🟡 Partner QA | migrations **`20260901120000`**–**`20260901170000`** + **`20260902100000`** + **`20260909100000`** (Connect recovery deduction); fail saga seller refund + receivables | `checkout/[id]` 驗四行 · `/admin/grading` **待追償** tab · seller pending banner · `MerchantOrderDetailView` gross/deduction/net | [plan](./follow-up/auth-escrow-v2/plan.md) · [backend](./follow-up/auth-escrow-v2/backend.md) · [frontend](./follow-up/auth-escrow-v2/frontend.md) · **blocks** Rewards Phase 2b until Phase D |
| Platform rewards v2 — Phase 2b (merchant_auth checkout coupons) | ✅ Ready (legacy) | ⏸ blocked | migration **`20260816120000`**, multicapture goods amount, grading-fail coupon restore, `checkout-coupons.ts` `useAuth` | `checkout/[id]` → `AuthEscrowReview` + auth toggle picker | [backend](./follow-up/platform-rewards-v2/backend.md) · [frontend](./follow-up/platform-rewards-v2/frontend.md) · **[QA](./follow-up/platform-rewards-v2/QA_CHECKLIST.md)** Part D · E2E `e2e/platform-rewards-phase2.spec.ts` (B2b) · **⏸ blocked on [auth-escrow-v2](./follow-up/auth-escrow-v2/plan.md)** — redo after v2 amount contract |
...
- **Stripe auth escrow (single capture):** 新鑑定單 `escrow_capture_model = 'single'` — Partner QA via [admin-grading PARTNER_HANDOFF](./follow-up/admin-grading/PARTNER_HANDOFF.md). Legacy multicapture 在途單（`NULL`）仍支援。
