...
| Member auth escrow (C2C 鑑定託管) | ✅ P0 Ready | 🟡 Partner QA（multicapture E2E） | `app/actions/checkout.ts`, `member-auth-checkout.ts`, `lib/checkout/*` | `checkout/[id]/*`, `MemberOrderDetailView` | [member-auth-checkout/backend](./follow-up/member-auth-checkout/backend.md) · [unified-checkout/backend](./follow-up/unified-checkout/backend.md) · **[partner handoff](./follow-up/admin-grading/PARTNER_HANDOFF.md)** — **出款：FPS T+3**（非 Connect） |
...
| Member rewards & gamification (check-in, coupons, auto-grant) | ✅ Ready | 🟡 Partial | `app/actions/rewards.ts`, **`lib/rewards/check-in-streak.ts`**, `lib/constants/rewards.ts`, `lib/rewards/mapUserRewardCoupon.ts`, migrations **`20260705180000`**–**`20260705188000`**, **`20260719150000`**–**`20260719170000`**, RPCs `execute_daily_check_in`, `get_gamification_stats_for_me`, **`get_reward_coupon_center`**, `get_unacknowledged_reward_grants`, **`fn_effective_check_in_streak`** | `CheckInCard.tsx`, `RewardUnlockedModal.tsx`, `RewardNotificationHost.tsx`, `UserProfileDashboardShell`, `/profile/user` hero PTS, `/profile/user/rewards` **4-tab coupon center** (wallet + **可解鎖** locked catalog); home merchant persona keeps check-in column empty (merchants cannot check in) | [backend](./follow-up/member-rewards-gamification/backend.md) · [frontend](./follow-up/member-rewards-gamification/frontend.md) |
...
| Platform rewards v2 — Phase 2 (merchant_direct checkout coupons) | ✅ Ready | ✅ Wired | migrations **`20260815120000`**, **`20260815130000`** (STABLE subsidy fix), `app/actions/checkout-coupons.ts`, `merchant-checkout.ts`, `execute-connect-payout.ts`, webhook coupon release | `checkout/[id]` → `CheckoutCouponPicker`, summary 平台優惠行 | [plan](./follow-up/platform-rewards-v2/plan.md) · [backend](./follow-up/platform-rewards-v2/backend.md) · [frontend](./follow-up/platform-rewards-v2/frontend.md) · **[QA](./follow-up/platform-rewards-v2/QA_CHECKLIST.md)** · E2E `e2e/platform-rewards-phase2.spec.ts` |
| Platform rewards v2 — Phase 2b (merchant_auth checkout coupons) | ✅ Ready | 🟡 Partner QA | migration **`20260816120000`**, multicapture goods amount, grading-fail coupon restore, `checkout-coupons.ts` `useAuth` | `checkout/[id]` → `AuthEscrowReview` + auth toggle picker | [backend](./follow-up/platform-rewards-v2/backend.md) · [frontend](./follow-up/platform-rewards-v2/frontend.md) · **[QA](./follow-up/platform-rewards-v2/QA_CHECKLIST.md)** Part D · E2E `e2e/platform-rewards-phase2.spec.ts` (B2b) |
| Platform rewards v2 — Phase 3 (flash campaigns) | ✅ Ready | 🟡 Partner QA | migration **`20260817120000`**, `app/actions/admin-reward-campaigns.ts`, `app/actions/reward-flash.ts`, wizard Step 3 + `AdminRewardCampaignsClient` | `/admin/campaigns` → **搶券檔期**; `/profile/user/rewards` → **限時搶券** | [backend](./follow-up/platform-rewards-v2/backend.md) · [frontend](./follow-up/platform-rewards-v2/frontend.md) · **[QA](./follow-up/platform-rewards-v2/QA_CHECKLIST.md)** Part E · E2E `e2e/platform-rewards-phase3.spec.ts` — ⏳ combined gate Parts D+E before Phase 4 |
...
| Merchant checkout — 平台 Stripe 收款 + Connect 撥款（B2C，Milestone 1–2） | ✅ Ready | 🟡 非鑑定 automatic ✅；鑑定 multicapture Partner QA | `checkout.ts`, `merchant-checkout.ts`, `lib/checkout/*`, migrations **`20260804120000`** | `checkout/[id]/*`, `MerchantOrderDetailView` | [merchant-checkout/backend](./follow-up/merchant-checkout/backend.md) · [unified-checkout/backend](./follow-up/unified-checkout/backend.md) · **[partner handoff](./follow-up/admin-grading/PARTNER_HANDOFF.md)** — **出款：Connect T+7** |
...
| Admin moderation & disputes（舉報仲裁 + 帳戶制裁） | ✅ Phase A–E+ Ready + **context dedup** | ✅ List + detail + chat + resolve + proxy + orders panel + **multi-room chat** | `app/actions/admin-moderation.ts`, `app/actions/reports.ts`, `lib/moderation/*`, `proxy.ts`, migrations **`20260806120000`**–**`20260812120000`** | `/admin/disputes`, `DisputeDetailClient.tsx`, `ModerationOrderContextPanel.tsx`, `/auth/suspended` | [backend](./follow-up/admin-moderation/backend.md#partner-manual-qaphase-e--e) · [frontend](./follow-up/admin-moderation/frontend.md#partner-manual-qaphase-e--e) · 📋 [Phase G subject history](./follow-up/admin-moderation/subject-history-plan.md) — Phase F：auto-escalation |
...
| Admin payouts（財務與結算） | ✅ Ready | 🟡 Partial | `app/actions/admin-payouts.ts`, `lib/admin-payouts/*`, migrations **`20260801120000`**, **`20260802120000`**, `app/api/cron/member-fps-payout-ready`, `scripts/dev/seed-fps-payout-requests.sql` | `/admin/payouts`, `FpsLedgerTab.tsx`, `MemberConnectLedgerTab.tsx` | [plan](./follow-up/admin-payouts/plan.md) · [backend](./follow-up/admin-payouts/backend.md) · [frontend](./follow-up/admin-payouts/frontend.md) — Merchant T+7 held ledger ✅; FPS ledger MVP ✅; auth multicapture → [partner handoff](./follow-up/admin-grading/PARTNER_HANDOFF.md) |
| Member FPS payout (C2C auth seller) | ✅ Ready | 🟡 Partner QA（multicapture 後段） | `app/actions/profile.ts` (`updateUserFpsId`), `app/actions/orders.ts` (payout fields), migrations **`20260802120000`**, **`20260803120000`**, cron **`/api/cron/member-fps-payout-ready`** | `FpsIdCollectDialog.tsx`, `MemberOrderDetailView.tsx`, `UserSettingsClient.tsx` | [backend](./follow-up/member-fps-payout/backend.md) · [frontend](./follow-up/member-fps-payout/frontend.md) · [e2e-checklist](./follow-up/member-fps-payout/e2e-checklist.md) · **[partner handoff](./follow-up/admin-grading/PARTNER_HANDOFF.md)** |
...
- **Stripe multicapture:** Account-level **online multicapture** enabled — partner QA via [admin-grading PARTNER_HANDOFF](./follow-up/admin-grading/PARTNER_HANDOFF.md). PI must use `request_multicapture: if_available`; **new auth orders only** (see `member-auth-checkout` / `merchant-checkout` follow-up).
