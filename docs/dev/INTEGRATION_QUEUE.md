...
| Admin payouts（財務與結算） | ✅ Ready | 🟡 Partial | `app/actions/admin-payouts.ts`, `lib/admin-payouts/*`, `lib/stripe/platform-today-inflow.ts`, migration **`20260801120000`** | `/admin/payouts`, `AdminPayoutsClient.tsx` | [plan](./follow-up/admin-payouts/plan.md) · [backend](./follow-up/admin-payouts/backend.md) · [frontend](./follow-up/admin-payouts/frontend.md) — Phase A: Merchant DB + Stripe balance; FPS tab mock; Phase B schema ✅; ⏳ Phase C cron + FPS wire |
