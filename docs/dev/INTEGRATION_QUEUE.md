...
| Admin moderation & disputes（舉報仲裁 + 帳戶制裁） | ✅ Phase A–E+ + **one open case** + **subject history G1–G5** | ✅ List + detail + chat + resolve + proxy + orders + **history panel** + **重犯 badge** + **E2E** | `app/actions/admin-moderation.ts`, `lib/moderation/*`, migrations **`20260806120000`**–**`20260814130000`** | `/admin/disputes`, `DisputeDetailClient.tsx`, `ModerationSubjectHistoryPanel.tsx` | [backend](./follow-up/admin-moderation/backend.md) · [frontend](./follow-up/admin-moderation/frontend.md) — Phase F：auto-escalation |
