...
| Merchant KYC flow (apply → admin review → Stripe Connect) | ✅ Ready | 🟡 Wizard + admin 表格 UI 已接駁；Stripe CTA + Finance 摘要已接 | `rpc_submit_merchant_kyc_application`（provisional merchant on submit）, `lib/kyc/merchant-gates.ts`, `lib/stripe/payout-ready.ts`, `app/actions/merchant-finance.ts`, migrations **`20260728100000`**–**`20260728150000`** | `MerchantApplyClient.tsx`（submit → `/profile/merchant`）, `MerchantOverviewClient.tsx`（KYC banner + Stripe CTA）, `MerchantFinanceClient.tsx` | [backend](./follow-up/merchant-kyc/backend.md) · [frontend](./follow-up/merchant-kyc/frontend.md) |
| Merchant checkout — 平台 Stripe 收款 + Connect 撥款（B2C，Milestone 1–2） | ✅ Ready | 🟡 T+7 hold 已接駁，E2E 待重測 | `merchant-checkout.ts`, `completeMerchantOrder` → `rpc_confirm_merchant_buyer_receipt`, `lib/merchant-order/execute-connect-payout.ts`, cron `merchant-connect-payout-ready`, migrations **`20260729100000`**–**`20260804120000`** | `checkout/[id]`, `MemberOrderDetailView`, `MerchantOrderDetailView`, `MerchantFinanceClient`, timelines | [backend](./follow-up/merchant-checkout/backend.md) · [frontend](./follow-up/merchant-checkout/frontend.md) · **[T+7 hold](./follow-up/merchant-connect-payout-hold/backend.md)** |
