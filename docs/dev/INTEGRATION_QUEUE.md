...
| Variable commission（平台佣金 SSOT） | ✅ Ready | ✅ Wired（commission only） | migration **`20260916120000`**, `fn_platform_commission_rate()`, `app/actions/admin-settings.ts`, `lib/platform/financial-config.ts` | `/admin/settings` commission · `MerchantOrderDetailView` pre-confirm estimate | [plan](./follow-up/variable-commission/plan.md) · [backend](./follow-up/variable-commission/backend.md) · [frontend](./follow-up/variable-commission/frontend.md) — appraisal/FPS settings mock |
