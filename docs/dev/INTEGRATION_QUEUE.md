...
| Merchant checkout — 平台 Stripe 收款 + Connect 撥款（B2C，Milestone 1–2） | ✅ Ready | 🟡 已接駁，E2E 待手測 | `merchant-checkout.ts`, `pending-payment-expiry.ts`, payout saga, `expire-merchant-pending-payment` cron, migrations **`20260729100000`**–**`20260803120600`**, `lib/marketplace/enrich-merchant-shipping.ts` | `checkout/[id]`（48h 倒數 + PaymentElement brand）, `ListingEditDialog` 附加運費, Buy Now 商戶直跳 checkout, trading 待付款 UI | [backend](./follow-up/merchant-checkout/backend.md) · [frontend](./follow-up/merchant-checkout/frontend.md) — 商戶自定運費；固定 8%；⏳ 動態費率 / refund |
