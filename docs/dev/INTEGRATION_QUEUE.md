...
| Member auth escrow (C2C 鑑定託管) | ✅ P0 Ready | 🟡 Unified checkout wired；鑑定 multicapture E2E ⏸ | `app/actions/checkout.ts`, `member-auth-checkout.ts`, `lib/checkout/*` | `checkout/[id]/*`, `MemberOrderDetailView` | [member-auth-checkout/backend](./follow-up/member-auth-checkout/backend.md) · [unified-checkout/backend](./follow-up/unified-checkout/backend.md) — **出款：FPS T+3**（非 Connect） |
...
| Merchant checkout — 平台 Stripe 收款 + Connect 撥款（B2C，Milestone 1–2） | ✅ Ready | 🟡 Unified wizard；非鑑定 automatic ✅ | `checkout.ts`, `merchant-checkout.ts`, `lib/checkout/*`, migrations **`20260804120000`** | `checkout/[id]/*`, `MerchantOrderDetailView` | [merchant-checkout/backend](./follow-up/merchant-checkout/backend.md) · [unified-checkout/backend](./follow-up/unified-checkout/backend.md) — **出款：Connect T+7** |
...
| Admin moderation & disputes（舉報仲裁 + 帳戶制裁） | ✅ Phase A–E+ Ready | ✅ List + detail + chat + resolve + proxy + orders panel | `app/actions/admin-moderation.ts`, `app/actions/listings.ts`, `lib/moderation/*`, `proxy.ts`, migrations **`20260806120000`**–**`20260810120000`** | `/admin/disputes`, `DisputeDetailClient.tsx`, `ModerationOrderContextPanel.tsx`, `/auth/suspended` | [backend](./follow-up/admin-moderation/backend.md#partner-manual-qaphase-e--e) · [frontend](./follow-up/admin-moderation/frontend.md#partner-manual-qaphase-e--e) — Phase F：auto-escalation |
