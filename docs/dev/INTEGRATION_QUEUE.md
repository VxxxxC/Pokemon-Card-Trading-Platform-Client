...
| Platform rewards v2 — Phase 4 (points redemption catalog) | 🟡 Ready | 🟡 Wired (baseline) | migration **`20260910130000`**, `rpc_list_points_redemption_catalog`, `rpc_redeem_points_catalog_item`, admin catalog sync | `/profile/user/rewards` → `PointsRedemptionSection` below Flash · `RewardActivityForm` 上架積分商城 | [phase-4-plan](./follow-up/platform-rewards-v2/phase-4-plan.md) · Vitest `points-redemption-catalog.integration.test.ts` (I-G*) + `redeem-points-catalog-action.test.ts` (I-G9) · E2E `platform-rewards-phase4.spec.ts` in `test:e2e:rewards-gate` |
