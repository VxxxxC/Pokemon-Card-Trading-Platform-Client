...
> **North star：** **SC-FX-ALL**（見 §5）= Ledger B **S2** 達標 + `test:staging:certify` 綠 + **SC-P0** + Partner M0。  
> **本表 `T-depth` 欄：** **Ledger A** — artifact 觸及 T0–T3；**唔等同** Partner-safe（見 [test-coverage-ssot.md](./test-coverage-ssot.md) §0.0）。  
> **主 SSOT：** [test-coverage-ssot.md](./test-coverage-ssot.md) v2.6（旅程 TC/J/SEC 與本表對照）  
...
### 0.5 雙表進度（Ledger A vs Ledger B）

| Ledger | 本表欄位 | 意思 | `check-staging-certification.sh` |
|--------|----------|------|----------------------------------|
| **A · T-depth** | `T-depth` | 目標深度 T0–T3 有 artifact + CI 觸點 | **預設模式**掃描（F-M/C/A/S 須 ☑） |
| **B · S2** | [test-coverage-ssot.md](./test-coverage-ssot.md) J-/TC- `進度` | Partner UI / eligibility 守衛 | **唔**掃描；由 **SC-FX-ALL** + certify + SC-P0 守 |

**唔好將 Ledger A 67/67 當 Staging 已認證。** Ledger B 現況約 **27/67 S2 ☑** — 見 SSOT §0.0。

---

## 1. Member（會員）

| ID | 功能 | 目標深度 | Artifact | CI | T-depth |
|----|------|----------|----------|-----|------|
...
† **Nightly†** = 要接入 `test:nightly:coverage` 或 gate 先可標 T-depth ☑（spec 已有未入 CI 仍 ☐）。  
‡ **T-depth ☑ / S2 ◐** — Ledger A 有 logic/integration 觸點；Ledger B 仍有缺口（HTTP route / CC-E2E）。見 SSOT TC-M01+ · L8。
...
| **F-S-02** | Cron HTTP（6 條） | T1 | `cron-routes`（RPC logic ☑）· **TC-M01–M06 HTTP ☐** | Nightly | ☑‡ |
...
| **F-S-09** | Config Contract parity | T1+T2 | CC 三件套 · registry（**CC-INT ☑ · CC-E2E L8 ☐**） | Rewards | ☑‡ |
...
### 5.1 Ledger A（T-depth — prerequisite）

| 匯總 ID | 條件 | T-depth |
|---------|------|---------|
| **SC-FX-M** | §1 Member 全行 T-depth ☑ | ☑ |
| **SC-FX-C** | §2 Merchant 全行 T-depth ☑ | ☑ |
| **SC-FX-A** | §3 Admin 全行 T-depth ☑ | ☑ |
| **SC-FX-S** | §4 System 全行 T-depth ☑ | ☑ |

**Ledger A 統計：** Member **26/26** · Merchant **13/13** · Admin **15/15** · System **13/13** — **67/67 T-depth ☑**（artifact 已登記；**唔等於 Partner-safe**）。

### 5.2 Ledger B + 執行（true exit）

| 匯總 ID | 條件 | 進度 |
|---------|------|------|
| **SC-FX-ALL** | Ledger B **S2** 達標（SSOT J-/TC- ~27/67 ☑）+ `test:staging:certify` 綠 + **SC-P0** + Partner M0 | ☐ |

**Ledger B 統計（2026-08-18）：** 約 **27/67 S2 ☑** · **6 ◐** · **34 ☐** — 見 [test-coverage-ssot.md](./test-coverage-ssot.md) §0.0 · §3。Partner P0：**0/8** — [partner-regression.md](./partner-regression.md)。
...
| 日期 | 變更 |
|------|------|
| 2026-08-18 | **v2.6：** `進度` → `T-depth`（Ledger A）· §0.5 / §5 雙表 · F-S-02 / F-S-09 ‡ 註明 |
