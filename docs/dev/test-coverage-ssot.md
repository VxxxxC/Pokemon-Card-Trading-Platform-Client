# Test Coverage & Solidity SSOT — v2.6

> **版本：** v2.6 · **更新：** 2026-08-18  
...
> **North star：** **SC-FX-ALL** = Ledger B（**S2**）達標 + `test:staging:certify` 綠 + **SC-P0** + Partner M0 = **Staging 可俾人用**。  
> **本文件：** 旅程（J-）、技術項（TC-）、矩陣、CC、安全 — 與功能表 ID 對照（registry §6）。

---

## 0. Staging 認證契約（v2.6 核心）
...
| 完成線？ | **Ledger B（S2）** 達標 + certify 綠 + **SC-P0** + M0 |
| v3 唔計？ | [v3-deferred.md](./v3-deferred.md) |

### 0.0 雙表進度（唔好混淆）

| Ledger | 欄位 / 位置 | 問咩 | 現況（2026-08-18） | 可否當 Staging 已認證 |
|--------|-------------|------|-------------------|----------------------|
| **A · T-depth** | registry `T-depth` 欄 | 有冇 artifact 觸及 T0–T3（spec 存在 + 某 CI job 有跑） | **67/67 ☑** | ❌ 唔夠 |
| **B · S2 Partner** | 本文件 J-/TC- 表 `進度` + `Solid` | Partner UI 真路徑（或 eligibility 矩陣 + Partner E2E） | **~27/67 ☑ · ~6 ◐ · ~34 ☐** | ✅ 必要條件之一 |

**常見誤讀（覆蓋率錯覺）：**

| 誤讀 | 真相 |
|------|------|
| 「67/67 ☑ = 全站 Partner-safe」 | Ledger A 只證明 **有 test 觸點**；Fixture-only / Manual / 大量 skip **唔計 S2** |
| 「CI 綠 = UI 冇事」 | L6 曾 **96 skipped**（§11）；S1 integration 綠 ≠ S2 Partner 綠（例：`member_auth` refund UI） |
| 「F-S-02 Cron ☑」 | Registry T-depth ☑ = RPC/`cron-routes` logic；**TC-M01–M06 HTTP route contract 仍 ☐**（L4） |

`bun run test:staging:certify --check-ssot` 預設只驗 **Ledger A**（F-M/C/A/S `T-depth` ☑）。**真正 exit** 見 [staging-certification.md](./staging-certification.md) **SC-FX-ALL**（Ledger B + certify + SC-P0）。

**現況（Ledger B）：** 約 **27/67** 旅程／功能 **S2 ☑**（+6 **◐**）— **未達 Staging 認證**（待 certify 綠 + SC-P0）。
...
| 符號 | 條件 |
|------|------|
| **☑** | `Solid ≥ Solid 目標` **且** `In CI` ∈ Gate / Nightly / Rewards **且** 對應 [staging-certification.md](./staging-certification.md) SC 可標 ☑ | **Ledger B（S2）** |
| **◐** | 有 spec 或 CI 有跑，但未達 S2（soak、Manual、缺 Partner E2E、CC 缺口、skip 過多） | Ledger B 未達標 |
| **☐** | 未開始或 Missing — **阻塞 SC-FX-ALL** | Ledger B 未達標 |

> Registry `T-depth` ☑（Ledger A）**唔等同** 上表 ☑。例：F-S-02 T-depth ☑（RPC logic）但 TC-M01 HTTP ☐ → Ledger B 仍 ◐。
...
| 日期 | 變更 |
|------|------|
| 2026-08-18 | **v2.6：** §0.0 雙表進度（Ledger A T-depth vs Ledger B S2）· 常見誤讀 · 釐清 27/67 vs 67/67 |
