...
> **North star：** **SC-FX-ALL** = Ledger B（**S2**）達標 + **`bun run test:staging:certify` 綠** + **SC-P0** + Partner **M0** = Staging 可俾人用。  
> **Partner UI：** [partner-regression.md](./partner-regression.md) · **技術細表：** [test-coverage-ssot.md](./test-coverage-ssot.md) v2.6
...
| # | 條件 | 驗證方式 |
|---|------|----------|
| **C1a** | Registry **Ledger A**：F-M/C/A/S `T-depth` 全 ☑ | `bun run test:staging:certify --check-ssot` |
| **C1b** | **SC-FX-ALL**（Ledger B S2 + 下述 C2–C6） | registry §5.2 · SSOT §0.0 |
...
### 2.0 功能層（Ledger A — prerequisite）

👉 **[system-feature-registry.md](./system-feature-registry.md)** — 67 項 in-scope 功能（Member 26 · Merchant 13 · Admin 15 · System 13）。`T-depth` 欄 = **Ledger A**（artifact 觸點）；**唔等同** S2 Partner-safe（見 SSOT §0.0）。

`check-staging-certification.sh` 掃描 **F-M/C/A/S `T-depth`** 欄（預設模式）。**SC-FX-ALL**（Ledger B exit）見 registry §5.2。
...
| 日期 | 變更 |
|------|------|
| 2026-08-18 | v2.6：C1 拆 C1a（Ledger A T-depth）/ C1b（SC-FX-ALL）· 雙表釐清 |
