...
// TODO: [database] Replace with Supabase query — fetch authenticated user's portfolio stats from `portfolios` table
...
// TODO: [database] Replace with Supabase query — fetch authenticated user's member level, XP, and rating from `profiles` table
const member = {
  levelTier: 3,
  xpCurrent: 2_040,
  xpRequired: 3_000,
...
// TODO: [database] Replace with Supabase query — fetch earned badges from `user_badges` join table
const badges = [
...
// TODO: [database] Replace with Supabase query — fetch reviews from `reviews` table where reviewee_id = current user, ordered by created_at DESC
const reviews = [
  {
    id: "rev-001",
    reviewer: "K.田中",
          ...
          <section aria-labelledby="activity-heading">
            ...
            <div className="bg-bg-card rounded-2xl border border-[rgba(237,232,224,0.08)] overflow-hidden">
              {recentActivity.map((tx, i) => (
                <div
                  key={tx.id}
                  className={`flex items-center gap-3 px-4 py-3 hover:bg-bg-elevated transition-colors ${i > 0 ? "border-t border-[rgba(237,232,224,0.08)]" : ""}`}
                >
                  ...
                  <div className="text-right shrink-0">
                    ...
                    <span
                      className={`font-mono text-[11px] ${tx.deltaDir === "up" ? "text-success" : "text-warning"}`}
                    >
                      {tx.deltaDir === "up" ? "▲" : "▼"} ¥
