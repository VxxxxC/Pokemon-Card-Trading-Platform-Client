...
const INITIAL_OWNED_CARDS: OwnedCard[] = [
  { id: "c-001", name: "Charizard ex SAR (噴火龍)",     set: "Pokémon 151",   cardNo: "sv2a-182", grade: "PSA 10",  grader: "PSA", purchasePrice: 2100, currentValue: 2250, status: "holding" },
  { id: "c-002", name: "Umbreon ex SAR (月亮伊布)",        set: "Night Wanderer",cardNo: "sv6a-109", grade: "BGS 9.5", grader: "BGS", purchasePrice: 1800, currentValue: 1900, status: "holding" },
  { id: "c-003", name: "Pikachu AR (皮卡丘)",            set: "Pokémon 151",   cardNo: "sv2a-215", grade: "CGC 9",   grader: "CGC", purchasePrice:  410, currentValue:  425, status: "holding" },
  { id: "c-004", name: "Eevee AR (伊布)",            set: "Pokémon 151",   cardNo: "sv2a-213", grade: "PSA 10",  grader: "PSA", purchasePrice:  280, currentValue:  310, status: "listed"  },
  { id: "c-005", name: "Mimikyu ex SAR (謎擬Q)",        set: "Pokémon 151",   cardNo: "sv2a-233", grade: "PSA 9",   grader: "PSA", purchasePrice: 1250, currentValue: 1400, status: "holding" },
];
...
function StatusPill({ status }: { status: OwnedCard["status"] }) {
  const map = {
    ...
    listed:  { label: "已上架",  className: "text-[#d4a574] bg-[rgba(212,165,116,0.12)]" },
    grading: { label: "鑑定中",  className: "text-[#10b981] bg-[rgba(16,185,129,0.12)]" },
  };
    ...
    <div className="space-y-6">
        ...
        <div className="bg-[#26211C] rounded-2xl border border-[rgba(212,165,116,0.20)] p-5 shadow-[0_2px_8px_rgba(0,0,0,0.40)]">
          <div className="flex items-start justify-between mb-4 gap-4 flex-wrap">
            <div>
              ...
              <p className="font-mono text-[13px] text-[#10b981] mt-2 inline-flex items-center gap-1 font-semibold">
                ▲ HK$ {portfolioSummary.unrealizedPnl.toLocaleString("en-HK")} (+{portfolioSummary.pnlPercent}% 未實現損益)
              </p>
            </div>
            <Link
              href="/marketplace"
          ...
          <div className="grid grid-cols-3 gap-3">
            {[
              ...
              { label: "已鑑定規格",     value: `${portfolioSummary.gradedCount} 張` },
              { label: "未鑑定 Raw",     value: `${portfolioSummary.rawCount} 張` },
            ].map(({ label, value }) => (
              <div key={label} className="bg-[#17130f] rounded-xl px-3 py-2.5 border border-[rgba(237,232,224,0.04)]">
                <p className="font-mono text-[10px] text-[#d4c4b7] mb-0.5">{label}</p>
                <p className="font-mono font-semibold text-[15px] text-[#eae1da]">{value}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="lg:grid lg:grid-cols-[1fr_320px] lg:gap-6 items-start">
        ...
        <section aria-labelledby="cards-heading" className="space-y-4">
          <div className="flex items-center justify-between flex-wrap gap-2">
            <h2 id="cards-heading" className="font-sans font-semibold text-[16px] text-[#eae1da]">
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-0.5 flex-wrap">
                      <p className="font-sans text-[13px] font-semibold text-[#eae1da] truncate">{card.name}</p>
                      <GraderBadge grader={card.grader} />
                      <StatusPill status={card.status} />
                    </div>
                    <p className="font-mono text-[11px] text-[#d4c4b7]">{card.cardNo} · {card.grade}</p>
                  </div>
                  ...
                  <div className="text-right shrink-0">
                    ...
                    <span className={`font-mono text-[11px] inline-flex items-center gap-0.5 ${pnlDir === "up" ? "text-[#10b981]" : "text-[#ef4444]"}`}>
                      {pnlDir === "up" ? "▲" : "▼"} HK$ {Math.abs(pnl).toLocaleString("en-HK")}
                    </span>
                  </div>
                </div>
              );
        ...
        <div className="space-y-4 mt-6 lg:mt-0">
          ...
          <div className="p-4 bg-[rgba(212,165,116,0.06)] border border-[#d4a574]/20 rounded-2xl space-y-1.5 font-sans">
            <h4 className="text-[12px] font-semibold text-[#d4a574]">💡 關於簽到防作弊防線</h4>
            <p className="text-[11px] text-[#d4c4b7] leading-relaxed">
              簽到系統時間戳一律標記為 `[SERVER_TIME_RESOLVED]`，禁止由客端本地時間提交以防止竄改時區刷積分。
            </p>
          </div>
        </div>
      </div>
      ...
      <section aria-labelledby="wishlist-heading" className="mt-8">
        <div className="flex items-center justify-between mb-4">
          <h2 id="wishlist-heading" className="font-sans font-semibold text-[16px] text-[#eae1da] flex items-center gap-2">
