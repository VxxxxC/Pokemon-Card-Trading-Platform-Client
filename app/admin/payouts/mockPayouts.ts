...
export const stripePlatformBalance = {
  available: 1284650,
  pending: 236800,
  todayIn: 87450,
...
export const STRIPE_LOG_PAYOUT_COUNT = 38;

// TODO: [Stripe Wiring] Replace mock data with real Stripe API call
// Target API: stripe.transfers.list({ limit, starting_after }) | Fallback: mock
export const STRIPE_LOG_TRANSFER_COUNT = 38;
...
export const MOCK_MERCHANT_FLOWS = initialMerchantFlows;
...
export const TRANSFER_MERCHANTS = [
  "HarutoCards Premium",
  "AikoRare Collection",
  "Daichi Rare Cards",
  "KuroGamer TCG",
...
export const makePayoutId = (seq: number) =>
  `po_1QxAbC${String(seq).padStart(22, "0")}XYZ`;
export const makeTransferId = (seq: number) =>
  `tr_1QxDeF${String(seq).padStart(22, "0")}XYZ`;

export const cyclePayoutStatus = (seq: number): StripeLogStatus => {
  const cycle = seq % 5;
...
export const cycleTransferStatus = (seq: number): StripeLogStatus => {
  const cycle = seq % 6;
...
export const makePayoutDate = (seq: number) => {
...
export const makeTransferDate = (seq: number) => {
