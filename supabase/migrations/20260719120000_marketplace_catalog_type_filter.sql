-- Marketplace search: catalog type filter + sealed grade filter fix
...
DROP FUNCTION IF EXISTS public.search_marketplace_products(
  text, text, text, text, text[], text[], public.catalog_type[], jsonb, numeric, numeric, text, integer, integer
);
...
CREATE OR REPLACE FUNCTION public.search_marketplace_products(
  ...
  p_catalog_types public.catalog_type[] DEFAULT NULL,
...
AS $$
  ...
  filtered_listings AS (
    ...
    WHERE l.status = 'active'
      ...
      AND (
        p_catalog_types IS NULL
        OR cardinality(p_catalog_types) = 0
        OR pc.type = ANY (p_catalog_types)
      )
