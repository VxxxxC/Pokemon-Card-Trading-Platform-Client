-- FPS account holder name (轉數快收款人姓名) for member auth seller payouts

ALTER TABLE public.profiles
    ADD COLUMN IF NOT EXISTS fps_name TEXT;

ALTER TABLE public.payout_requests
    ADD COLUMN IF NOT EXISTS fps_name_snapshot TEXT;

-- Re-define finalize RPC to snapshot fps_name; ready only when id + name present
...
DECLARE
    ...
    v_request_id UUID;
BEGIN
    SELECT mo.seller_id, mo.final_price, p.fps_id, p.fps_name
    INTO v_seller_id, v_final_price, v_fps_id, v_fps_name
