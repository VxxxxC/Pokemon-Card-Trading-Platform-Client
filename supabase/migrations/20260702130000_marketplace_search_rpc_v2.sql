-- Marketplace search RPC v2: grading, seller_persona, structured catalog filters, pagination meta

CREATE INDEX IF NOT EXISTS idx_listings_active_grading
  ON public.listings (grading_company, grading_score)
  WHERE status = 'active';

CREATE INDEX IF NOT EXISTS idx_listings_active_seller_persona
  ON public.listings (seller_persona, use_authentication)
  WHERE status = 'active';

CREATE INDEX IF NOT EXISTS idx_product_catalog_set_card
  ON public.product_catalog (set_code, card_number);
...
GRANT EXECUTE ON FUNCTION public.search_marketplace_products(
  text, text, text, text[], text[], jsonb, numeric, numeric, text, integer, integer
)
