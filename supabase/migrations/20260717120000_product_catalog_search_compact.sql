-- Fast catalog identifier search: precomputed compact/canonical columns + index.
-- Replaces slow per-row plpgsql catalog_card_identifier_matches in search_product_catalog.

ALTER TABLE public.product_catalog
  ADD COLUMN IF NOT EXISTS id_compact text
    GENERATED ALWAYS AS (
      public.compact_alphanumeric(
        coalesce(display_id, '') || coalesce(set_code, '') || coalesce(card_number, '')
      )
    ) STORED,
  ADD COLUMN IF NOT EXISTS id_canonical text
    GENERATED ALWAYS AS (
      public.canonical_card_search_key(
        coalesce(display_id, '') || ' ' || coalesce(set_code, '') || ' ' || coalesce(card_number, '')
      )
    ) STORED;

CREATE INDEX IF NOT EXISTS idx_product_catalog_id_compact
  ON public.product_catalog (id_compact text_pattern_ops);
