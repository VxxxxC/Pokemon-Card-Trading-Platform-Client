-- Integration/e2e only: force merchant payout edge states for retry guard tests.
...
REVOKE ALL ON FUNCTION public.rpc_e2e_set_merchant_order_payout_retry_test_state(UUID, TEXT)
    FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.rpc_e2e_set_merchant_order_payout_retry_test_state(UUID, TEXT)
    TO service_role;
