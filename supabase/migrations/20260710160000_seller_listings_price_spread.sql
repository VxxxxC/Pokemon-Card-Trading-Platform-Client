-- Seller listing search: join market reference + RPC-computed price spread %.

CREATE OR REPLACE FUNCTION public.compute_price_vs_market_pct(
  p_listing_price numeric,
  p_market_avg_price numeric
)
RETURNS numeric
LANGUAGE sql
IMMUTABLE
AS $$
  SELECT CASE
    WHEN p_market_avg_price IS NOT NULL
     AND p_market_avg_price > 0
     AND p_listing_price IS NOT NULL
     AND p_listing_price > 0
    THEN round(
      ((p_listing_price - p_market_avg_price) / p_market_avg_price * 100)::numeric,
      1
    )
    ELSE NULL
  END;
...
GRANT EXECUTE ON FUNCTION public.compute_price_vs_market_pct(numeric, numeric)
