-- Timeline: expose Stripe transfer / source payout refs for recovery_applied events.
...
BEGIN
    ...
    FROM (
        SELECT
            ...
            NULL::TEXT AS stripe_transfer_id,
            NULL::TEXT AS source_payout_order_number,
            NULL::TEXT AS source_payment_intent_id
        ...
        SELECT
            ...
            END AS amount_hkd,
            CASE
                WHEN ml.transaction_type = 'grading_fail_recovery_applied'::public.transaction_type THEN
                    COALESCE(
                        NULLIF(trim(ml.stripe_transfer_id), ''),
                        payout_ref.stripe_transfer_id
                    )
                ELSE NULL
            END AS stripe_transfer_id,
            CASE
                WHEN ml.transaction_type = 'grading_fail_recovery_applied'::public.transaction_type
                    THEN payout_ref.source_payout_order_number
                ELSE NULL
            END AS source_payout_order_number,
            CASE
                WHEN ml.transaction_type = 'grading_fail_recovery_applied'::public.transaction_type
                    THEN payout_ref.source_payment_intent_id
                ELSE NULL
            END AS source_payment_intent_id
        ...
        LEFT JOIN LATERAL (
            SELECT
                COALESCE(
                    NULLIF(trim(pl.stripe_transfer_id), ''),
                    NULLIF(trim(po.stripe_transfer_id), '')
                ) AS stripe_transfer_id,
                po.order_number AS source_payout_order_number,
                po.stripe_payment_intent_id AS source_payment_intent_id
            FROM public.merchant_ledgers pl
            INNER JOIN public.merchant_orders po ON po.id = pl.order_id
            WHERE ml.transaction_type = 'grading_fail_recovery_applied'::public.transaction_type
              AND pl.merchant_id = ml.merchant_id
              AND pl.transaction_type = 'payout'::public.transaction_type
              AND po.transferred_at IS NOT NULL
              AND pl.created_at >= ml.created_at - INTERVAL '10 minutes'
              AND pl.created_at <= ml.created_at + INTERVAL '180 days'
            ORDER BY ABS(EXTRACT(EPOCH FROM (pl.created_at - ml.created_at)))
            LIMIT 1
        ) payout_ref ON true
