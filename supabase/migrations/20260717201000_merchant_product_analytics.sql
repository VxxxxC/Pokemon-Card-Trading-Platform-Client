-- Per-product merchant analytics: sales, views, offers time-series + paginated order history
...
BEGIN
  ...
  IF v_merchant_id IS NULL OR p_product_id IS NULL THEN
    RETURN NULL;
  END IF;
  ...
  series_rows AS (
    ...
    FROM buckets b
    LEFT JOIN interval_orders io
      ON io.event_at >= b.bucket_start
    ...
    (
      SELECT jsonb_build_object(
        ...
        'marketLowestPrice', COALESCE((
          ...
          WHERE mps.product_id = p_product_id
        ), 0),
        'totalViews', COALESCE((
  ...
  RETURN jsonb_build_object(
    'product', COALESCE(v_product, jsonb_build_object('id', p_product_id, 'name', '未知商品', 'skuNo', '—', 'imageUrl', '')),
...
GRANT EXECUTE ON FUNCTION public.get_merchant_product_analytics(uuid, text, integer, integer) TO authenticated;
