-- Merchant checkout delivery snapshot (B2C buyer recipient details at payment prepare).

ALTER TABLE public.merchant_orders
    ADD COLUMN IF NOT EXISTS sf_locker_code TEXT,
    ADD COLUMN IF NOT EXISTS sf_address TEXT,
    ADD COLUMN IF NOT EXISTS buyer_phone TEXT,
    ADD COLUMN IF NOT EXISTS meetup_detail TEXT,
    ADD COLUMN IF NOT EXISTS buyer_remark TEXT;
...
BEGIN
    ...
    IF p_shipping_method = 'sf' THEN
        IF v_sf_locker_code IS NULL OR v_buyer_phone IS NULL THEN
            RAISE EXCEPTION '請填寫順豐自提櫃代碼及聯絡電話。';
        END IF;
    ELSIF p_shipping_method = 'meetup' THEN
        IF v_meetup_detail IS NULL THEN
            RAISE EXCEPTION '請填寫面交地點與時間備註。';
        END IF;
    END IF;
    ...
    SET
        ...
        sf_locker_code = CASE
            WHEN p_shipping_method = 'sf' THEN v_sf_locker_code
            ELSE NULL
        END,
        sf_address = CASE
            WHEN p_shipping_method = 'sf' THEN v_sf_address
            ELSE NULL
        END,
        buyer_phone = CASE
            WHEN p_shipping_method = 'sf' THEN v_buyer_phone
            ELSE NULL
        END,
        meetup_detail = CASE
            WHEN p_shipping_method = 'meetup' THEN v_meetup_detail
            ELSE NULL
        END,
