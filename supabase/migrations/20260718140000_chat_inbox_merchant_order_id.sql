-- Include merchant_order_id in chat inbox / thread RPC payloads for B2C order flows.
      ...
      FROM (
        SELECT DISTINCT ON (cm.room_id)
          jsonb_build_object(
            ...
            'merchant_order_id', cm.merchant_order_id,
    ...
    'messages', COALESCE((
      SELECT jsonb_agg(
        jsonb_build_object(
          ...
          'merchant_order_id', page_cm.merchant_order_id,
