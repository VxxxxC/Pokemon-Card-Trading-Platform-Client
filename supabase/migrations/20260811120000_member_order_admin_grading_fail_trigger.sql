-- Allow admin session RPCs (rpc_prepare_auth_grading_fail / rpc_finalize_auth_grading_fail)
-- to UPDATE member auth orders without being buyer/seller. SECURITY DEFINER RPCs still
-- run triggers with the caller's auth.uid(); service_role already bypasses.
...
BEGIN
    ...
    IF COALESCE(OLD.use_authentication, false) THEN
        ...
        -- Admin grading fail prepare (void saga + legacy refund prepare)
        IF auth.uid() IS NOT NULL AND public.is_admin() THEN
            ...
            -- Admin grading fail finalize (void saga + legacy stripe refund finalize)
            IF OLD.status = 'pending'
