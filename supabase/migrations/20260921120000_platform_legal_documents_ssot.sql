...
-- Seeds platform_terms / platform_privacy; removes orphan fps_payout_config row.
...
6. 其他
使用本平台即表示您已閱讀並同意本條款及私隱政策（/privacy）。如有疑問請透過平台客服聯絡我們。$terms$
    )
  ),
  (
    'platform_privacy',
...
5. 您的權利與聯絡
您可要求查閱、更正或刪除個人資料（受法律及合約限制）。聯絡方式請見平台公告或客服渠道。另請參閱服務條款（/terms）。$privacy$
    )
  )
ON CONFLICT (key) DO NOTHING;

-- Orphan row: batch schedule is code SSOT in lib/admin-payouts/fps-batch-config.ts
DELETE FROM public.platform_settings WHERE key = 'fps_payout_config';
