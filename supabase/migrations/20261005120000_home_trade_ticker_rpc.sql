-- Homepage trade ticker: anonymized recent completed trades for public marquee.
...
REVOKE ALL ON FUNCTION public.rpc_list_home_trade_ticker(INTEGER) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.rpc_list_home_trade_ticker(INTEGER) TO anon, authenticated, service_role;

COMMENT ON FUNCTION public.rpc_list_home_trade_ticker(INTEGER) IS
    'Public homepage marquee: latest completed member + merchant orders (no PII).';
