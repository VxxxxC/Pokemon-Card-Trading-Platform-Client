-- Enforce one active offer per buyer per listing; allow re-offer only after rejection/cancellation
...
BEGIN
  ...
  -- 1b. 每位買家對同一掛單僅能有一筆有效出價；賣家拒絕或取消後方可再次出價
  IF EXISTS (
    SELECT 1
    FROM public.offers o
    INNER JOIN public.chat_rooms r ON r.id = o.room_id
