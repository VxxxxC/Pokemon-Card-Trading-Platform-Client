-- Merchant trading orders: paginated search RPC for seller dashboard
...
IMMUTABLE
AS $$
  SELECT p_escrow_status = 'payment_held'::public.escrow_state;
$$;

CREATE OR REPLACE FUNCTION public.fn_merchant_order_is_auth_in_progress(
  p_escrow_status public.escrow_state,
  p_requires_authentication boolean
)
RETURNS boolean
...
DROP POLICY IF EXISTS merchant_orders_participant_read ON public.merchant_orders;

CREATE POLICY merchant_orders_participant_read
    ...
    WHERE
      (
        ...
        OR (
          ...
          AND (
            (p.include_payment AND sm.escrow_status = 'payment_held'::public.escrow_state)
            OR (
              p.include_auth
  ...
  facet_counts AS (
    SELECT
      ...
      (
        ...
        FROM search_matched sm
        WHERE
          public.fn_merchant_order_is_open(sm.escrow_status)
