-- Fix anon/guest product detail order book: count public reviews via SECURITY DEFINER helper
-- instead of direct transaction_reviews access under SECURITY INVOKER RPC.

CREATE OR REPLACE FUNCTION public.fn_count_public_reviews_for_persona(
  p_reviewee_id uuid,
  p_reviewee_persona public.review_persona
)
RETURNS integer
LANGUAGE sql
STABLE
...
AS $$
  ...
  WHERE r.reviewee_id = p_reviewee_id
    AND r.reviewee_persona = p_reviewee_persona
    AND r.is_public = true;
$$;

GRANT EXECUTE ON FUNCTION public.fn_count_public_reviews_for_persona(
  uuid,
...
AS $$
  ...
  filtered_listings AS (
    SELECT
      ...
      public.fn_count_public_reviews_for_persona(
        l.seller_id,
        l.seller_persona::text::public.review_persona
